...
module SeparateComputationData
...
function solve_phasefield!(material, cv, dh, ch, a, a_old, Δt, states, states_old,l_factor,max_val_pp,sum_y,n_obs,P,sensor_locs)
    y = sum_y./n_obs
...
function element_routine_phasefield!(Ke, fe::AbstractVector, m::MicroMorphicElasticPhaseField, cvs, cell, ae::AbstractVector, ae_old, Δt, dh, state, state_old, l_factor, max_val_pp)
    ...
    for q_point in 1:getnquadpoints(cvu)
        ...
        for (i, I) in pairs(dofrange_d)
            ...
            fe[I] += (Gcl_cw2* (∇δNd ⋅ ∇d))*dΩ - (material.α*(ϕ-d)*δNd)*dΩ 
...
function calculate_phasefieldl_fac(::AT2_FM, m::MicroMorphicElasticPhaseField, Ψ⁺, Ψ⁻, d::T, ⁿϕ, l_factor=1, max_val_pp=1) where {T}
    return min(max((2 * Ψ⁺ + m.α * d) / (2 * Ψ⁺ + m.α + m.Gc / m.l), convert(T, ⁿϕ)), one(T).*max_val_pp)
end




function solve_displacement!(material, cv, dh, ch, a, a_old, Δt, states, states_old,sum_y,n_obs,P,sensor_locs,pred_cov)
    y = sum_y./n_obs
    n_sens = length(sensor_locs)
    σ_e =0.0004
    C_e = σ_e^2* Matrix(I, n_sens, n_sens)
    ...
    while true; newton_itr += 1          
        ...
        r = P*a - y  #residual data
        C_e_scaled = C_e .* 50
        grad_J_data = P' * (C_e_scaled\r)
        hess_J_data = P' * (C_e_scaled\P)
        lhs = Kd'*Kd + hess_J_data +1e-8*I
        rhs = - (Kd'*fd + grad_J_data)

        grad_J_data_aug = P' *(λ + (C_e_scaled\r))
        hess_J_data_aug = P' * (C_e_scaled\P)
        lhs_aug = Kd'*Kd + hess_J_data_aug +1e-8*I
        rhs_aug = - (Kd'*fd + grad_J_data_aug)
...
function element_routine_displacement!(Ke, fe::AbstractVector, m::MicroMorphicElasticPhaseField, cvs, cell, ae::AbstractVector, ae_old, Δt, dh, state, state_old)
    ...
    for q_point in 1:getnquadpoints(cvu)
        ...
        ⁿϕ = old_qp_state.ϕ
        Ψ⁺, Ψ⁻ = split_energy(material, ϵ)
        ϕ = calculate_phasefield_l_fac(material, Ψ⁺, Ψ⁻, d)

        gϕ = degradation_function(material, d)
